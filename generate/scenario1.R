...
get_data <- function(n0, n1, scenario) {
  ...
  trt <- trt %>%
    mutate(Y = 1 + 2 * X + X^2 + rnorm(n1, mean = 0, sd = 1/sqrt(2)))
  ...
  con <- con %>%
    mutate(Y = 1 + 2 * X + X^2 + rnorm(n0, mean = 0, sd = 1/sqrt(2)))
