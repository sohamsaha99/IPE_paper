...
# \int y w^{\star}((1-h)A + hB; x) dP_B
find_theta_cross <- function(h, categorized1, categorized2) {
  result <- solve_lp(h, categorized1, categorized2)
  sum(result$wts_min *
      categorized2$proportion *
      categorized2$mean_y)
}
...
find_f0min <- function(h, categorized1, categorized2) {
  result <- solve_lp(h, categorized1, categorized2)
...
dir.create("results/example/v5/", recursive = TRUE, showWarnings = FALSE)
write.csv(results, "results/example/v5/results.csv", row.names = FALSE)
# write.csv(results_25, "results/example/v5/results_25.csv", row.names = FALSE)
results <- read.csv("results/example/v5/results.csv", header = TRUE)
# results_25 <- read.csv("results/example/v5/results_25.csv", header = TRUE)
# results <- rbind(results_25, results)
# Visualize results
...
# Print $\sqrt{\tn}\EE \left( \tilde{\theta}_{\text{min}} - \theta_{\text{min}} \right)$,
# the scaled difference $\sqrt{\tn} \EE \left(\sum_x (u_{J_A}(x) - \hat{u}_{J_A,A}) f_1(x)\mu(x)\right)$ and
# the scaled expectation of the correction term $\sqrt{\tn} \EE \left( \frac{\partial}{\partial h} \left[ \int_{\mathcal{X}\times \mathcal{Y}} y w^\star\left((1-h)\hat{P}_A + h \hat{P}_B; x\right) d\hat{P}_B \right]_{h=0^+} \right)$ 
results$scaled_correction <- sqrt(results$n1) * (results$theta_min_cv2 - results$theta_min_cv) # derivative
results$scaled_difference <- NA # difference due to vertex error
for (i in seq_len(nrow(results))) {
  w_est <- c(results$w01[i], results$w02[i], results$w03[i])
  J <- results$optimal_index[i]
  w_J <- verts_true[J, paste0("w", 1:3)]
  results$scaled_difference[i] <- sqrt(2 * results$n1[i]) *
    sum((w_est - w_J) * probabilities * regression_func(sample_space))
}
results %>% group_by(n1) %>%
  summarise(
            scaled_bias = mean(scaled_error_cv),
            scaled_correction = mean(scaled_correction),
            scaled_difference = mean(scaled_difference)
  )
cat("For scaled difference, remember to shift n1 appropriately.\n")
...
arrow_raw <- verts_opt %>%
  ...
  slice_tail(n = 20) %>%
  mutate(w_deriv_norm = sqrt(w01_deriv^2 + w02_deriv^2)) %>%
  mutate(f0_deriv_norm = sqrt(f01_deriv^2 + f02_deriv^2))
...
# --- Plot in the f0 space ---
p4 <- ggplot() +
  # True polytope (triangle) boundary
  geom_polygon(
    data  = verts_true_plot,
    aes(x = f01, y = f02, group = 1),
    fill  = "orange",
    # color = "orange",
    linewidth = 1,
    alpha = 0.5
  ...
  stat_density_2d(
    data = verts_dat,
    aes(x = f01, y = f02, group = vertex, fill = after_stat(level)),
    geom = "polygon",
    alpha = 0.2,
    fill = "steelblue3"
  ) +
  # Add mean of estimated vertices
  geom_point(data = verts_dat %>% group_by(vertex) %>% summarise(f01 = mean(f01), f02 = mean(f02)),
  aes(f01, f02), size = 3.5) +
  ...
  guides(fill = "none", color = "none") +
  theme_mydefault(base_size = 20) +
  xlim(c(0.0, 0.75)) + ylim(c(0.2, 0.6))
...
p5 <- ggplot() +
  ...
  stat_density_2d(
                  data = verts_opt,
                  aes(x = f01, y = f02, group = optimal_index, fill = after_stat(level)),
                  alpha = 0.2,
                  geom = "polygon",
                  fill = "steelblue3"
                  ) +
  ...
  guides(fill = "none", color = "none") +
  theme_mydefault(base_size = 19) +
  xlim(c(0.0, 0.7)) + ylim(c(0.2, 0.5))
...
ggsave("results/example/v5/A.svg", plot = p1,
       width = 7.5, height = 10, units = "in")
ggsave("results/example/v5/B.svg", plot = p2,
       width = 15, height = 5, units = "in")
ggsave("results/example/v5/C.svg", plot = p3,
       width = 15, height = 5, units = "in")
ggsave("results/example/v5/D.svg", plot = p4,
       width = 15, height = 5, units = "in")
ggsave("results/example/v5/E.svg", plot = p5,
       width = 15, height = 5, units = "in")
